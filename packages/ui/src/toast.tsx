...
interface ToastItem
  extends Required<Pick<ToastPayload, "title" | "variant" | "duration">> {
  id: string;
  description?: string;
}
    ...
    <ToastContext.Provider value={value}>
      ...
      {portalTarget
        ? createPortal(
            <div
              aria-live="polite"
              className="pointer-events-none fixed right-4 top-4 z-[120] flex w-[min(92vw,24rem)] flex-col gap-2"
            >
              {toasts.map((item) => (
                <div
                  className={cn(
                ...
                >
                  <div className="flex items-start gap-2">
                    <span
                      className={cn(
                        "mt-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-semibold uppercase",
                        variantBadgeStyles[item.variant]
                      )}
                    >
                      {item.variant}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      {item.description ? (
                        <p className="mt-0.5 text-xs text-current/90">
                          {item.description}
                        </p>
                      ) : null}
                    </div>
