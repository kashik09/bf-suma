          ...
          >
            {slides.map((slide) => (
              <div className="w-full shrink-0 px-1 sm:w-1/2 xl:w-1/3" key={slide.kind === "all" ? "all" : slide.category.id}>
                {slide.kind === "all" ? (
                  <Link
                    className="group relative isolate block overflow-hidden rounded-2xl bg-gradient-to-br from-brand-700 via-brand-600 to-sky-700 p-5 shadow-soft"
                    href="/shop"
                  >
