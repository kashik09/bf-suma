-- Add delivery_zone column to orders table for cash-on-delivery eligibility
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS delivery_zone TEXT NULL;

-- Update RPC function to accept and store delivery_zone
...
begin
  ...
  if p_currency <> 'UGX' then
    return query select
      ...
      null::jsonb,
      jsonb_build_object('currency', jsonb_build_array('Only UGX is supported.'));
    return;
  end if;
  ...
  begin
    ...
    v_response := jsonb_build_object(
      ...
      'total', v_order_total,
      'currency', coalesce(v_order_currency, 'UGX')
    );
