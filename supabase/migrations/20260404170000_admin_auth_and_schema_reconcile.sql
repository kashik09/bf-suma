create table if not exists public.admin_users (
  ...
  constraint admin_users_name_non_empty check (char_length(trim(name)) > 0),
  constraint admin_users_email_non_empty check (char_length(trim(email)) > 0)
);
...
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    execute 'revoke all on public.admin_users from anon';
  ...
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    execute 'revoke all on public.admin_users from authenticated';
  ...
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    execute 'grant all on public.admin_users to service_role';
